...
<template>
  <nav :class="containerClassName" :style="style" aria-label="Main navigation">
    <!-- <div :class="[
      ...
      <span :class="['logo-content', 'inline-flex items-center justify-center', 'w-[120px] h-full']"
        :style="{ '--logo-max-height': '60%', '--logo-max-width': '100%' }">
        <img v-if="typeof logo === 'string'" :src="logo as string" alt="Logo"
    ...
    <button type="button" :class="[
      ...
      'will-change-transform'
    ]" @click="handleToggle" :aria-label="menuAriaLabel" :aria-pressed="isMenuOpen" :style="{ background: menuBg }">
      <span class="block mx-auto rounded-[2px] menu-line" :style="{
        width: '26px',
        height: '2px',
        background: menuContentColor,
        transform: isMenuOpen ? 'translateY(4px) rotate(45deg)' : 'none'
      }" />
      <span class="block mx-auto rounded-[2px] menu-line short" :style="{
        marginTop: '6px',
        width: '26px',
        height: '2px',
        background: menuContentColor,
  ...
  <div v-if="showOverlay" ref="overlayRef" :class="[
    'bubble-menu-items',
    ...
    <ul :class="[
      ...
      'gap-x-0 gap-y-1',
      'pointer-events-auto'
    ]" role="menu" aria-label="Menu links">
      <li v-for="(item, idx) in menuItems" :key="idx" role="none"
        :class="['pill-col', 'flex justify-center items-stretch', '[flex:0_0_calc(100%/3)]', 'box-border']">
        <RouterLink @click="handleCloseMenu" role="menuitem" :to="item.href" :aria-label="item.ariaLabel || item.label"
          :class="[
            'pill-link',
            'w-full',
            'rounded-[999px]',
